...
/**
 * Gets the Command which executed the process
 */
...
/**
 * Reads the /proc Dir and gets the LIst of All processes
 * All processess will be integers so has the logic to check which all have all integers and then caches
 * them in a vector which it returns 
 * 
 */
...
string ProcessParser::getVmSize(string pid)
{
    ...
    while(std::getline(stream, line)){
        ...
        if (line.compare(0, dField.size(),dField) == 0) {
 ...
 * CPU Usage
 * 
 * Once we acquire the correct lines of data from proc/[PID]/stat, we use it to calculate CPU usage. 
 * Because the timing is measured in CPU clock ticks, not seconds, we need to convert the data from the file
 */
